...
static bool fill_input_buffer_and_record(j_decompress_ptr cinfo)
{
	loadimage_jpeg_client_data *client_data = (loadimage_jpeg_client_data *)cinfo->client_data;
	struct loadexif_client_data *data = client_data->exif_data;
	
	bool ret = data->orgf.fill_input_buffer(cinfo);
	
	if (!ret) return ret;
	
	size_t bytes_to_copy = cinfo->bytes_in_buffer;
	if (bytes_to_copy > (LOADEXIF_BUFFER_SIZE - data->inputsize)) {
		bytes_to_copy = LOADEXIF_BUFFER_SIZE - data->inputsize;
	}
	
	if (bytes_to_copy) {
		memcpy (data->inputdata + data->inputsize, cinfo->next_input_byte, bytes_to_copy);
